...
def process_dataset(data_path='dataset/UserBehavior.csv'):
    ...
    with open(data_path, 'r') as f:
        ...
        #dec_03_data = [row for row in enumerate(f) if row[4].startwith('2017-12-03') & row[3] == 'pv']
            if timestamp>=1512230400:
                dec_03_data.extend([infos[0],infos[1],infos[3]])
    ...
    print(all_data_set)
    train_size=int(len(all_data_set)*0.8)
    train_data=list(all_data_set)[:train_size]
    test_data=list(all_data_set)[train_size:]
